...
export default function AdminServices() {
  ...
  const saveDate=async(date:string)=>{const entry=services.find(s=>s.date===date);if(!entry)return;try{await fetch('/api/services/'+eventId+'/dates',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({date,services:entry.entries})});notifications.show({title:'Services saved for '+date,color:'green'});load();}catch(e:any){notifications.show({title:'Error',message:e.message,color:'red'});}};
