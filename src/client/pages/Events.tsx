...
import { Container, Card, Text, Title, Group, Button, Stack, Modal, TextInput, SimpleGrid, Badge, Checkbox, Divider, ActionIcon, Skeleton } from '@mantine/core';
...
export default function Events() {
  ...
  if (loading) return (
    <Container pb={70}><Breadcrumbs items={[{label:'Super Admin',href:'/admin'},{label:'Events'}]} />
      <Skeleton height={36} width={200} mb="md" /><SimpleGrid cols={{base:1,sm:2}}>{[1,2].map(i=><Skeleton key={i} height={160} radius="md" />)}</SimpleGrid>
    </Container>
  );
  ...
  return (
    <Container pb={70}>
      ...
      {events.length === 0 ? (
        <EmptyState icon={<IconBuilding size={64} />} title="No events" description="Create your first event to get started." actionLabel="Create Event" onAction={() => { setEditEvent(null); setForm({name:'',description:'',start_date:'',end_date:'',event_code:'',hotel_name:'',hotel_address:'',contact_person:'',contact_phone:''}); setSelectedHotels([]); setCreateNewHotel(false); open(); }} />
      ) : (
        ...
        {events.map((e: any, i:number) => (
          <MotionCard key={e.id} padding="md" radius="md" withBorder initial={{opacity:0,y:20}} animate={{opacity:1,y:0}} transition={{delay:i*0.05}}>
            <Group justify="space-between" mb="xs">
              ...
              <Group gap={4}>
                <ActionIcon size="sm" variant="subtle" onClick={() => { setEditEvent(e); setForm({name:e.name,description:e.description||'',start_date:e.start_date,end_date:e.end_date,event_code:e.event_code,hotel_name:'',hotel_address:'',contact_person:'',contact_phone:''}); setSelectedHotels([]); open(); }}><IconEdit size={14} /></ActionIcon>
                <ActionIcon size="sm" variant="subtle" color="red" onClick={async () => { if(confirm('Delete?')){ await api.events.delete(e.id); load(); }}}><IconTrash size={14} /></ActionIcon>
              </Group>
            </Group>
            ...
            <Group gap={4} mt={4}>
              {(e.hotels || []).map((h: any) => (
                <Badge key={h.id} size="sm" variant="light" color="blue" leftSection={<IconMapPin size={10} />}>
                  {h.name}
                  {h.staff_code && <span style={{cursor:'pointer',marginLeft:4}} onClick={(ev) => { ev.stopPropagation(); copyCode(h.staff_code); }}>📋</span>}
                </Badge>
              ))}
            </Group>
      ...
      <Modal opened={opened} onClose={()=>{close();setEditEvent(null);}} title={editEvent?'Edit Event':'New Event'} fullScreen>
        <Stack>
          {!editEvent && (<>
            ...
            {hotels.map((h:any) => (<Checkbox key={h.id} label={`${h.name} — ${h.address}`} checked={selectedHotels.includes(h.id)} onChange={()=>setSelectedHotels(prev=>prev.includes(h.id)?prev.filter(id=>id!==h.id):[...prev,h.id])} />))}
            ...
            {createNewHotel && (<Stack><TextInput label="Hotel Name" required value={form.hotel_name} onChange={e=>setForm({...form,hotel_name:e.target.value})} /><TextInput label="Address" value={form.hotel_address} onChange={e=>setForm({...form,hotel_address:e.target.value})} /><TextInput label="Contact Person" value={form.contact_person} onChange={e=>setForm({...form,contact_person:e.target.value})} /><TextInput label="Contact Phone" value={form.contact_phone} onChange={e=>setForm({...form,contact_phone:e.target.value})} /></Stack>)}
            <Divider />
          </>)}
          ...
          <Button onClick={async () => {
            ...
            try {
              if(editEvent){ await api.events.update(editEvent.id,{name:form.name,description:form.description,start_date:form.start_date,end_date:form.end_date}); }
                ...
                if(!hotelIds.length){ notifications.show({title:'Error',message:'Select at least one hotel',color:'red'}); setSaving(false); return; }
            ...
            setSaving(false);
          }} loading={saving}>{editEvent?'Update Event':'Create Event'}</Button>
        </Stack>
      </Modal>
    </Container>
  );
