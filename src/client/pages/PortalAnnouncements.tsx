...
export default function PortalAnnouncements() {
  ...
  useEffect(()=>{if(token){fetch('/api/portal/'+token+'/announcements').then(r=>r.json()).then((data)=>{setAnnouncements(data);data.forEach((a:any)=>{if(!a.is_read)fetch('/api/portal/'+token+'/announcements/'+a.id+'/read',{method:'POST'});});}).catch(()=>{});}},[token]);
