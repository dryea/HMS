...
export default function AdminSchedulePlanner() {
  ...
  const load=async()=>{if(!eventId)return;try{const ev=await(await fetch('/api/events/'+eventId)).json();setEvent(ev);const start=new Date(ev.start_date),end=new Date(ev.end_date),ds:string[]=[];for(let d=new Date(start);d<=end;d.setDate(d.getDate()+1))ds.push(d.toISOString().split('T')[0]);setDates(ds);if(!activeDate&&ds.length)setActiveDate(ds[0]);}catch{}try{setSessions(await(await fetch('/api/sessions/'+eventId)).json());}catch{}try{setLocations(await(await fetch('/api/locations/'+eventId)).json());}catch{}};
