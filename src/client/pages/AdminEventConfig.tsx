...
export default function AdminEventConfig() {
  ...
  const load=async()=>{if(!eventId)return;try{const ev=(await(await fetch('/api/events/'+eventId)).json()) as any;setForm({name:ev.name,description:ev.description||'',start_date:ev.start_date,end_date:ev.end_date});}catch{}try{const b=(await(await fetch('/api/branding/'+eventId)).json()) as any;setBranding({logo_url:b.logo_url||'',banner_color:b.banner_color||'#1C1B1B',accent_color:b.accent_color||'#8C4A48'});}catch{}try{setCustomTypes(await(await fetch('/api/event-config/service-types/'+eventId)).json());}catch{}};
  useEffect(()=>{load();},[eventId]);
  const saveEvent=async()=>{try{await fetch('/api/events/'+eventId,{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify(form)});notifications.show({title:'Saved',message:'Event details saved.',color:'green'});}catch(e:any){notifications.show({title:'Error',message:e.message,color:'red'});}};
  const saveBranding=async()=>{try{await fetch('/api/branding/'+eventId,{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify(branding)});notifications.show({title:'Branding saved',message:'Branding saved successfully.',color:'green'});}catch(e:any){notifications.show({title:'Error',message:e.message,color:'red'});}};
  const addCustomType=async()=>{if(!newTypeName)return;try{await fetch('/api/event-config/service-types',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({event_id:eventId,name:newTypeName})});setNewTypeName('');load();notifications.show({title:'Service type added',message:'Custom service type added successfully.',color:'green'});}catch(e:any){notifications.show({title:'Error',message:e.message,color:'red'});}};
  const uploadLogo=async(file:File|null)=>{if(!file)return;const fd=new FormData();fd.append('file',file);try{const r=(await(await fetch('/api/event-config/upload/logo/'+eventId,{method:'POST',body:fd})).json()) as any;setBranding({...branding,logo_url:r.url});notifications.show({title:'Logo uploaded',message:'Logo uploaded successfully.',color:'green'});}catch(e:any){notifications.show({title:'Error',message:e.message,color:'red'});}};
