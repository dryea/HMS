...
import { IconPlus, IconBed, IconUser, IconChevronDown, IconChevronRight, IconTrash, IconUsers, IconEdit } from '@tabler/icons-react';
...
export default function HotelRooms() {
  ...
  return (
    <Container pb={70}>
      <Breadcrumbs items={[{label:'Super Admin',href:'/admin'},{label:'Events',href:'/admin/events'},{label:event?.name||'',href:'/admin/events/'+eventId},{label:'Rooms',href:'/admin/events/'+eventId+'/rooms'},{label:hotel?.name||''}]} />
      <Group mb="md">
        <Title order={3}>{hotel?.name||'Hotel'} Rooms</Title>
        <Select size="xs" placeholder="Switch hotel" data={allHotels.map(h=>({value:h.id,label:h.name}))} value={hotelId} onChange={v=>v&&v!==hotelId&&nav('/admin/events/'+eventId+'/hotels/'+v+'/rooms')} searchable />
      </Group>
      <Group mb="md">
        <Button size="xs" variant="light" color="teal" leftSection={<IconUsers size={14}/>} onClick={()=>nav('/admin/events/'+eventId+'/hotels/'+hotelId+'/participants')}>People</Button>
      ...
      {Object.entries(byWing).map(([wing, wingRooms]:[string,any]) => (
        <MotionCard key={wing} withBorder mb="sm" padding="sm" radius="md" initial={{opacity:0,y:10}} animate={{opacity:1,y:0}}>
          ...
          <Collapse in={expandedWings[wing]}>
            {wingRooms.map((room:any) => (
              <MotionCard key={room.id} withBorder mt="xs" padding="xs" radius="sm" layout>
                <Group mb={4}>
                  <IconBed size={14}/><Text size="sm" fw={500}>Room {room.room_number}</Text>
                  {room.floor && <Badge size="xs">Floor {room.floor}</Badge>}
                  <StatusDot status={room.status} />
                  <ActionIcon size="xs" variant="subtle" onClick={()=>{setEditRoom(room);setEditForm({room_number:room.room_number,floor:room.floor||'',wing:room.wing||'',status:room.status,room_type_id:room.room_type_id||''});openEdit();}}><IconEdit size={12}/></ActionIcon>
                  <ActionIcon size="xs" color="red" variant="subtle" onClick={async()=>{if(confirm('Delete room?')){await api.rooms.delete(room.id);load();}}}><IconTrash size={12}/></ActionIcon>
                </Group>
                <div className="bed-grid">
                  {(room.beds||[]).map((bed:any)=>(
                    <Paper key={bed.id} p={4} withBorder radius="sm" className={'bed-card'+(bed.is_occupied?' occupied':' vacant')}
                      onClick={()=>{if(!bed.is_occupied)setAssignModal(bed.id);else if(bed.pid){api.participants.unassignBed(bed.pid).then(load);}}}>
      ...
      <Modal opened={opened} onClose={close} title="Generate Rooms">
        <Stack>
          ...
          <Button onClick={async()=>{try{await api.rooms.generate({event_id:eventId,hotel_id:hotelId,...gen});notifications.show({title:'Rooms created',color:'green'});close();load();}catch(e:any){notifications.show({title:'Error',message:e.message,color:'red'});}}}>Generate</Button>
        </Stack>
      </Modal>
      <Modal opened={!!assignModal} onClose={()=>setAssignModal(null)} title="Assign Bed">
        <Stack>
          {unassigned.map((p:any)=>(<Button key={p.id} variant="light" fullWidth onClick={async()=>{try{await api.participants.assignBed(p.id,assignModal!);notifications.show({title:'Assigned',color:'green'});setAssignModal(null);load();}catch(e:any){notifications.show({title:'Error',message:e.message,color:'red'});}}}>{p.name}</Button>))}
