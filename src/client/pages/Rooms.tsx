    ...
    <div className="page-container">
      ...
      <Modal opened={editOpened} onClose={closeEdit} title="Edit Room" centered>
        <div className="flex flex-col gap-12">
          ...
          <button className="md3-btn" style={{width:'100%'}} onClick={async()=>{try{await fetch('/api/rooms/'+editRoom.id,{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify(editForm)});notifications.show({title:'Room updated',message:'The room details have been updated.',color:'green'});closeEdit();load();}catch(e:any){notifications.show({title:'Error',message:e.message,color:'red'});}}}>Save</button>
        </div>
      </Modal>
      ...
      <Modal opened={opened} onClose={close} title="Generate Rooms" centered>
        <div className="flex flex-col gap-12">
          ...
          <button className="md3-btn" style={{width:'100%'}} onClick={async()=>{if(!gen.hotel_id){notifications.show({title:'Error',message:'Select a hotel',color:'red'});return;}try{await api.rooms.generate({event_id:eventId,...gen});notifications.show({title:'Rooms created',message:'Rooms have been generated.',color:'green'});close();load();}catch(e:any){notifications.show({title:'Error',message:e.message,color:'red'});}}}>Generate</button>
        </div>
      </Modal>
      ...
      <Modal opened={!!assignModal} onClose={()=>setAssignModal(null)} title="Assign Bed" centered>
        <div className="flex flex-col gap-8">
          {unassigned.filter((p:any)=>!p.bed_label).map((p:any)=>(
            <button key={p.id} className="md3-btn-text" style={{width:'100%',justifyContent:'flex-start'}} onClick={async()=>{try{await api.participants.assignBed(p.id,assignModal!);notifications.show({title:'Assigned',message:'Participant assigned to bed successfully.',color:'green'});setAssignModal(null);load();}catch(e:any){notifications.show({title:'Error',message:e.message,color:'red'});}}}>{p.name}</button>
          ))}
